...
<script type="text/javascript">
	...
	function getSpu(ppid){
		$("#select_spu").empty();
		$("#select_spu").append("<option>请选择</option>")
		$.getJSON(("js/json/spu_"+ppid+".js"),function(data){
			
			$(data).each(function(i,json){
				$("#select_spu").append("<option value="+json.id+">"+json.shp_mch+"</option>")
			});
		});
	}
</script>
...
二级菜单:<select id="select_class2" name="flbh2" onchange="loadSkuList(this.value)"></select>
商标: <select id="select_class3" name="pp_id" onchange="getSpu(this.value)"></select><br>
商品信息:<select id="select_spu" name="shp_id" onchange="getshpid(this.value)"></select>
<hr>
</body>
</html>